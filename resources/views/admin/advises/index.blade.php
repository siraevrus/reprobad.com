...
@section('content')
    @include('admin.components.header', ['title' => 'Список советов', 'route' => 'advises'])

    <form method="GET" action="{{ route('admin.advises.index') }}" class="mb-4 flex gap-2">
        <input
            type="text"
            name="search"
            value="{{ $search ?? '' }}"
        ...
        @if(!empty($search))
            <a href="{{ route('admin.advises.index') }}" class="rounded border border-gray-300 px-4 py-2 text-gray-700">Сбросить</a>
        @endif
    </form>
            ...
            @foreach($resources as $resource)
                ...
                <tr class="border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <td class="py-3 px-6 text-center">
                        @include('admin.components.controls', ['route' => 'advises', 'resource' => $resource])
                    </td>
                </tr>
            @endforeach
            </tbody>
